...
export const FormCreateRecipe = ({ ingredientsData }: props) => {
  ...
  const handleSubmit: FormProps["onFinish"] = (values) => {
    startTransition(async () => {
      try {
        const res = await axios.post(process.env.NEXT_PUBLIC_API_URL + "/api/recipe/create", values.resep);
        if (res.status === 201) {
        ...
        {(fields, { add, remove }) => (
          <div style={{ display: "flex", rowGap: 16, flexDirection: "column" }}>
            ...
            <Button type="dashed" onClick={() => add()} block>
              {fields.length < 1 ? "Masukan Resep" : "Tambah Resep Lain"}
            </Button>
          </div>
        )}
      </Form.List>
